...
- Render side, in [signlab_drs-pipeline](https://github.com/Amsterdam-Humanities-Labs/signlab_drs-pipeline): `services/crop_fix.py` reads `/videoFix/crop_fixes.json`, crops again and calls `api.php?action=update_status`. `.htaccess` rewrites that URL to `api.php?action=crop_fixes_json`. `tools/reprocess_all_fixes.py` renders every entry again and only reads the queue. Both send `X-Api-Token`.

## Login
- `require_auth.php` guards every action of `api.php`, `crop_fixes_json` included. Without a login it returns HTTP 401 JSON; `index.html` then shows a "log in first" link.
- Browser: a SignCollect login session, checked by `menu_beta/php_api/session.php` (as in signlab_camera-control's `require_login.php`).
- Scripts: header `X-Api-Token: <VIDEOFIX_TOKEN>`. A wrong token gets 401.
...
- `VIDEOFIX_TOKEN=` in `<webroot>/.env`, read with `sc_env()` from signcollect-lib, or the `VIDEOFIX_TOKEN` environment variable if the lib is missing. See `videofix.env.example`. Without it, token requests get 401.
- Browser login needs `menu_beta/php_api/session.php` (signlab_signCollect-v2). Without it, every browser request gets 401.
